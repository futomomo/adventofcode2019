...
void prgopAddMem(program* p)
{
  ...
  if(augend >= p->prgSize
      ...
      || outmem >= p->prgSize)
  {
    //printf("ADD: Opcode trying to access out of bounds memory, skipping execution.\n");
    p->instPtr += 4;
    return;
  }
...
void prgopMulMem(program* p)
{
  ...
  outmem       = p->prg[p->instPtr+3];
  if(multiplicand >= p->prgSize
      || multiplier >= p->prgSize
      || outmem >= p->prgSize)
  {
    //printf("MUL: Opcode trying to access out of bounds memory, skipping execution.\n");
    p->instPtr += 4;
    return;
  }
...
program* copyPrg(const program* src)
{
  ...
  dest->prgSize = src->prgSize;
  dest->instPtr = src->instPtr;
  dest->prg = malloc(sizeof(int)*(src->prgSize));
  if(!dest->prg)
  {
    printf("Failed to allocate program prg memory.\n");
    return NULL;
  }

  memcpy(dest->prg, src->prg, (sizeof(int)*(src->prgSize)));
  return dest;
}
